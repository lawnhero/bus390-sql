...
def sidebar():
    with st.sidebar:
        ...
        with st.container(horizontal=True):
            if st.session_state.get("chat_history"):
                st.download_button(
                    ...
                    file_name=f"sql_ta_chat_{datetime.now():%Y%m%d_%H%M}.txt",
                    mime="text/plain",
        ...
        # 3. What the tutor can help with — driven by the same TOPICS list as
        #    the topic pills, so the two can never disagree.
        st.space("small")
        with st.expander("The nine modules", icon=":material/checklist:"):
            st.markdown("\n".join(
