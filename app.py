...
def initialize_chains(_retriever):
    ...
    try:
        chains_dict = {
            "course": chains.rag_chain(llms.claude_haiku, _retriever),
            "exercise": chains.exercise_chain(llms.claude_sonnet),
            "chat": chains.chat_chain(llms.openai_gpt4o_mini),
            "explain": chains.explain_chain(llms.openai_gpt4o),
            "debug": chains.debug_chain(llms.claude_haiku),
        }
...
def main():
    st.set_page_config(
        page_title="BUS 390 SQL Virtual TA",
        page_icon=":material/school:",
        layout="wide",
    )
    st.title("BUS 390 SQL Virtual TA")
    ...
    if pending and pending.get("needs") == "attempt":
        chat_placeholder = "Paste your SQL and the error message here..."
    elif pending:
        chat_placeholder = "Pick a topic above, or type one here..."
    else:
        chat_placeholder = "Ask a SQL or course question — no question is too basic..."
        ...
        with st.chat_message("AI", avatar=TA_AVATAR):
            ...
            try:
                ...
                if tool_call.tool_calls and tool_call.tool_calls[0]["name"] in chain_dict:
                    ...
                    route, args = "chat", {"query": user_query}
    ...
    if topic_choice:
        ...
        _cancel_pending()
        run_turn(INTENT_QUERY_TEMPLATES[intent].format(topic=topic_choice))
        ...
        if pending and pending.get("needs") == "topic":
            ...
            if len(text.split()) <= 6 and not text.endswith("?"):
                run_turn(INTENT_QUERY_TEMPLATES[intent].format(topic=text))
            else:
                run_turn(text)
        elif pending and pending.get("needs") == "attempt":
            _cancel_pending()
            run_turn(f"Please help me debug this SQL:\n{text}")
        else:
            run_turn(text)
